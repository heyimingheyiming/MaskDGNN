...
def load(nodes_num):
    ...
    train_n_feat = read_e_feat(train_n_feat_path)  # 读取训练节点特征
    test_n_feat = read_e_feat(test_n_feat_path)  # 读取测试节点特征

    train_e_feat = read_e_feat(train_e_feat_path)  # 读取训练边特征
    test_e_feat = read_e_feat(test_e_feat_path)  # 读取测试边特征

    num = 0
    train_graph = read_graph(train_path, nodes_num, num)  # 读取训练图
    num = num + len(train_graph)  # 更新 num 以便读取测试图时使用
    test_graph = read_graph(test_path, nodes_num, num)  # 读取测试图
    return train_graph, train_e_feat, train_n_feat, test_graph, test_e_feat, test_n_feat  # 返回所有读取的数据

def load_r(name):
    path = "dataset/" + name  # 数据集的基础路径
    path_ei = path + '/' + 'edge_index/'  # 边索引路径
    path_nf = path + '/' + 'node_feature/'  # 节点特征路径
    path_ef = path + '/' + 'edge_feature/'  # 边特征路径
    path_et = path + '/' + 'edge_time/'  # 边时间路径

    edge_index = read_npz(path_ei)  # 读取边索引
    edge_feature = read_npz(path_ef)  # 读取边特征
    node_feature = read_npz(path_nf)  # 读取节点特征
    edge_time = read_npz(path_et)  # 读取边时间

    nodes_num = node_feature[0].shape[0]  # 获取节点数量
