...
def train(args, model, optimizer, device, graph_t, graph_l, logger, n):

    best_param = {'best_auc': 0, 'best_mrr': 0, 'best_state': None, 'best_s_dw': None}
    earl_stop_c = 0
    epoch_count = 0
    ...
    for epoch in range(args.epochs):
        ...
        while i < (n - args.window_num):
            ...
            if i >= (n - args.window_num):
                break
            graph_train = graph_t[i: i + args.window_num]
            graph_train1 = graph_l[i: i + args.window_num]
            i = i + 1
        ...
        if all_mrr > best_param['best_mrr']:
            ...
            earl_stop_c += 1
            if earl_stop_c == args.early_stop:
                break
    return best_param
