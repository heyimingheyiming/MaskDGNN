...
def test(graph_l, model, args, logger, n, S_dw, device):
    ...
    for idx, g_test in enumerate(graph_test):
        ...
        mrr, rl1, rl3, rl10 = report_rank_based_eval_meta(model, graph_test[idx + 1], graph_test[idx + 1].node_feature, fast_weights)
    ...
    avg_rl10 /= len(graph_test) - 1
    logger.info({'avg_auc': avg_auc, 'avg_acc': avg_acc, 'avg_mrr': avg_mrr, 'avg_rl10':avg_rl10})
    return avg_auc